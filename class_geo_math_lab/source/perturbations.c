...
int perturbations_total_stress_energy(
  ...
  if (_scalars_) {
    ...
    if (pba->has_cdm == _TRUE_) {
if (ppt->geo_mode == 0) {
  ppw->delta_rho += ppt->geo_xi * ppw->pvecback[pba->index_bg_rho_cdm]*y[ppw->pv->index_pt_delta_cdm];
}
else {
  ppw->delta_rho += ppt->geo_mu * ppw->pvecback[pba->index_bg_rho_cdm]*y[ppw->pv->index_pt_delta_cdm];
}
...
if (ppt->has_source_delta_m == _TRUE_) {
if (ppt->geo_mode == 0) {
  delta_rho_m += ppt->geo_xi * ppw->pvecback[pba->index_bg_rho_cdm] * y[ppw->pv->index_pt_delta_cdm];
  rho_m += ppt->geo_xi * ppw->pvecback[pba->index_bg_rho_cdm];
}
else {
  delta_rho_m += ppt->geo_mu * ppw->pvecback[pba->index_bg_rho_cdm] * y[ppw->pv->index_pt_delta_cdm];
  rho_m += ppt->geo_mu * ppw->pvecback[pba->index_bg_rho_cdm];
}  
}
